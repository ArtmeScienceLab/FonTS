...
def load_flow_model(name: str, device: str | torch.device = "cuda", hf_download: bool = True):
    # Loading Flux
    print("Init model")
    ckpt_path = '/tiamat-NAS/shiwenda/0_new/models/pretrained/FLUX.1-dev/flux1-dev.safetensors' #configs[name].ckpt_path
    if (
        ckpt_path is None
...
def load_t5(device: str | torch.device = "cuda", max_length: int = 512) -> HFEmbedder:
    ...
    return HFEmbedder("/tiamat-NAS/shiwenda/0_new/models/pretrained/FLUX.1-dev/text_encoder_2", max_length=max_length, torch_dtype=torch.bfloat16).to(device)

def load_clip(device: str | torch.device = "cuda") -> HFEmbedder:
    # return HFEmbedder("openai/clip-vit-large-patch14", max_length=77, torch_dtype=torch.bfloat16).to(device)
    return HFEmbedder("/tiamat-NAS/shiwenda/0_new/models/pretrained/FLUX.1-dev/text_encoder", max_length=77, torch_dtype=torch.bfloat16).to(device)


def load_ae(name: str, device: str | torch.device = "cuda", hf_download: bool = True) -> AutoEncoder:
    ckpt_path = '/tiamat-NAS/shiwenda/0_new/models/pretrained/FLUX.1-dev/ae.safetensors' # configs[name].ae_path
    if (
        ckpt_path is None
        and configs[name].repo_id is not None
